...
router.get('/', (req, res) => {
    res.json(getEnvelopes(req.user.householdId));
});

router.get('/suggestion', (req, res) => {
    res.json(getContributionSuggestions(req.user.householdId));
});
...
router.post('/', (req, res) => {
    ...
    const info = db
        ...
        .run(req.user.householdId, name, type || 'CUSTOM', target_amount || 0);
    res.status(201).json(db.prepare('SELECT * FROM envelopes WHERE id = ?').get(info.lastInsertRowid));
});

router.post('/:id/deposit', (req, res) => {
    try {
        const envelope = depositToEnvelope(req.user.householdId, req.params.id, Number(req.body.amount), req.body.note);
        res.json(envelope);
    } catch (err) {
...
router.post('/:id/withdraw', (req, res) => {
    try {
        const envelope = withdrawFromEnvelope(req.user.householdId, req.params.id, Number(req.body.amount), req.body.note);
        res.json(envelope);
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
